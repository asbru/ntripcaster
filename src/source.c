...
void
source_login (connection_t *con, char *expr)
{
  ...
  do
    {
      ...
      if (splitc (line, expr, '\n') == NULL)
        {
          ...
          if ((go_on > 0)
              && (!source->protocol == icy_e && source->type == encoder_e))
            {
      ...
      if (line[0])
        xa_debug (2, "DEBUG: Source line [%d]: [%s] [%s]", go_on--, command,
                  arg);
      ...
      if (ice_strncmp (command, "SOURCE", 6) == 0
          || ice_strncmp (command, "POST", 4) == 0) // Kun
        {
          ...
          get_mount_location_from_file (info.mountposfile,
                                        source->audiocast.mount, &source->pos);
        }
      else if (strncasecmp (command, "Source-Agent", 12) == 0
               || strncasecmp (command, "User-Agent", 10) == 0)
        {
  ...
  if (connected)
    {
      ...
      write_log (
          LOG_DEFAULT,
          "Accepted encoder on mountpoint %s from %s. %d sources connected",
  ...
  write_log (
      LOG_DEFAULT,
      "WARNING: Thread exiting in source_login(), this should not happen");
  thread_exit (0);
}

/* This function is started as a new thread for every connected and accepted
   source. Either it gets killed by another thread, using the kick_connection
   (thiscon,..), which should set the connected value to SOURCE_KILLED, and let
   this thread exit on it's own with the close_connection at the end. Or it
   kills itself, when the source dies, and then it should call kick_connection
   (thiscon,..) on itself, setting the value of connected to SOURCE_KILLED, and
   exit through close_connection () */
void *
source_func (void *conarg)
{
  ...
  while (thread_alive (mt)
         && ((source->connected == SOURCE_CONNECTED)
             || (source->connected == SOURCE_PAUSED)))
    {
          ...
          while ((clicon = avl_traverse (source->clients, &trav)) != NULL)
            {
              ...
              if (source->connected == SOURCE_KILLED
                  || source->connected == SOURCE_PAUSED)
                break;
  ...
  if (!req || !req->path || !req->host)
    {
      write_log (LOG_DEFAULT,
                 "WARNING: find_mount_with_req called with NULL request!");
      return NULL;
    }

  xa_debug (1, "DEBUG: Looking for [%s] on host [%s] on port %d", req->path,
            req->host, req->port);
  ...
  while ((con = avl_traverse (info.sources, &trav)) != NULL)
    {
      ...
      xa_debug (2, "DEBUG: Looking on mount [%s]",
      ...
      if (con->food.source->audiocast.mount[0] == '/')
        ...
        {
          if ((ice_strcmp (con->food.source->audiocast.mount, req->path + 1)
               == 0)
...
add_chunk (connection_t *con)
{
  ...
  if (con->food.source->chunk[con->food.source->cid].clients_left > 0)
    {
#ifndef OPTIMIZE
      xa_debug (2, "DEBUG: Kicking trailing clients [%d] on id %d",
                con->food.source->chunk[con->food.source->cid].clients_left,
                con->food.source->cid);
#endif
      thread_mutex_lock (&info.double_mutex);
  ...
  do
    {
      ...
      len = recv (con->sock,
                  con->food.source->chunk[con->food.source->cid].data
                      + read_bytes,
                  SOURCE_READSIZE - read_bytes, 0);

      xa_debug (
          5, "DEBUG: Source received %d bytes in try %d, total %d, errno: %d",
          len, tries, read_bytes, errno);
      ...
      if ((len == 0) || ((len == -1) && (!is_recoverable (errno))))
        {
          if (info.client_timeout > 0
              && con->food.source->connected != SOURCE_KILLED)
            {
              ...
              /* Sleep for client_timeout seconds. If during that time this
              source is set to SOURCE_KILLED, return false */
              ...
              if (pending_source_signoff (con))
                {
                  ...
                  thread_mutex_lock (&con->food.source->mutex);
                  kick_connection (con,
                                   "Client timeout exceeded, removing source");
                  thread_mutex_unlock (&con->food.source->mutex);
                  thread_mutex_unlock (&info.double_mutex);
                  return;
                }
  ...
  while ((((double)read_bytes < ((double)SOURCE_READSIZE * 3.0) / 4.0)
          && (tries < (READ_TIMEOUT / READ_RETRY_DELAY))));

  if (read_bytes <= 0)
    {
      write_log (LOG_DEFAULT,
                 "Didn't receive data from source after %d microseconds, "
                 "assuming it died...",
                 tries * READ_RETRY_DELAY);
      ...
      if (info.client_timeout > 0)
        {
          /* Sleep for client_timeout seconds. If during that time this source
           * is set to SOURCE_KILLED, return false */
          if (pending_source_signoff (con))
            {
              thread_mutex_lock (&info.double_mutex);
              thread_mutex_lock (&con->food.source->mutex);
...
write_chunk (source_t *source, connection_t *clicon)
{
  ...
  for (i = 0; i < 2; i++)
    {
      ...
      len = source->chunk[clicon->food.client->cid].len
            - clicon->food.client->offset;

      xa_debug (5,
                "DEBUG: write_chunk(): Try: %d, writing chunk %d to client "
                "%d, len(%d) - offset(%d) == %d",
                i, clicon->food.client->cid, clicon->id,
      ...
      if (len < 0 || source->chunk[clicon->food.client->cid].len == 0)
        {
#ifndef OPTIMIZE
          xa_debug (5, "DEBUG: write_chunk: Empty chunk [%d] [%d]",
                    source->chunk[clicon->food.client->cid].len,
                    clicon->food.client->offset);
#endif
          ...
          continue; /* Perhaps for some reason the source read a zero sized
                       chunk but the next one is ok */
        }
      ...
      if (write_bytes < 0)
        {
#ifndef OPTIMIZE
          xa_debug (5, "DEBUG: client: [%2d] errors: [%3d]", clicon->id,
                    client_errors (clicon->food.client));
#endif
          if (is_recoverable (0 - write_bytes))
            continue;
          break; /* Safe to assume that the client is kicked out due to socket
                    error */
        }
      ...
      if (write_bytes + clicon->food.client->offset
          >= source->chunk[clicon->food.client->cid].len)
        {
          source->chunk[clicon->food.client->cid].clients_left--;
          clicon->food.client->cid = (clicon->food.client->cid + 1) % CHUNKLEN;
...
#ifndef OPTIMIZE
          xa_debug (5, "DEBUG: client %d only read %d of %d bytes", clicon->id,
                    write_bytes,
                    source->chunk[clicon->food.client->cid].len
                        - clicon->food.client->offset);
#endif
  ...
  xa_debug (4,
            "DEBUG: client %d tried %d times, now has %d errors %d chunks "
            "behind source",
            clicon->id, i, client_errors (clicon->food.client),
            source->cid < clicon->food.client->cid
                ? source->cid + CHUNKLEN - clicon->food.client->cid
                : source->cid - clicon->food.client->cid);
...
kick_clients_on_cid (source_t *source)
{
  ...
  unsigned int max
      = avl_count (source->clients) * avl_count (source->clients) + 2;
  ...
  xa_debug (5, "DEBUG: In function kick_clients_on_cid. Source has %d clients",
            max);
#endif
  while (max >= 0)
    {
      ...
      if (client_errors (clicon->food.client) >= (CHUNKLEN - 1)
          && clicon->food.client->alive != CLIENT_DEAD)

        {
          kick_connection (clicon,
                           "Client cannot sustain sufficient bandwidth");
      ...
      if (client_errors (clicon->food.client) >= (CHUNKLEN - 1))
        {
          kick_connection (
              clicon,
              "Too many errors (client not receiving data fast enough)");
        }
    }
      ...
      if (clicon->food.client->alive == CLIENT_MOVE)
        {
          ...
          /* No zero_trav() here cause it's not removed from the list
           * (close_connection() does that) */
        }
...
write_data (connection_t *clicon, source_t *source)
{
  ...
  if (source->chunk[clicon->food.client->cid].len - clicon->food.client->offset
      <= 0)
    return 0;

  write_bytes
      = sock_write_bytes_or_kick (clicon->sock, clicon,
                                  &source->chunk[clicon->food.client->cid]
                                       .data[clicon->food.client->offset],
                                  source->chunk[clicon->food.client->cid].len
                                      - clicon->food.client->offset);

#ifndef OPTIMIZE
  xa_debug (4,
            "DEBUG: client %d in write_data(). Function write() returned %d "
            "of %d bytes, client on chunk %d (+%d), source on chunk %d",
            clicon->id, write_bytes,
            source->chunk[clicon->food.client->cid].len
                - clicon->food.client->offset,
            clicon->food.client->cid, clicon->food.client->offset,
            source->cid);
#endif
  if (write_bytes < 0)
    return 0 - errno;
...
const char source_types[5][16]
    = { "encoder", "pulling relay", "on demand relay", "file transfer",
        "unknown source" };
...
source_write_to_client (source_t *source, connection_t *clicon)
{
  ...
  if (!clicon || !source)
    {
      xa_debug (1,
                "WARNING: source_write_to_client() called with NULL pointers");
      return;
    }
...
source_get_new_clients (source_t *source)
{
  ...
  while ((clicon = pool_get_my_clients (source)))
    {
      xa_debug (1, "DEBUG: source_get_new_clients(): Accepted client %d",
                clicon->id);
