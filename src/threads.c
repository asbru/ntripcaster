...
#ifdef DEBUG_MEMORY
void
thread_mem_check (mythread_t *thread)
{
  ...
  while ((mt = avl_traverse (info.mem, &trav)))
    {
      if (mt->thread_id == thread->id)
        xa_debug (1,
                  "WARNING: %d bytes allocated by thread %d at line %d in %s "
                  "not freed before thread exit",
                  mt->size, thread->id, mt->line, mt->file);
    }
...
icethread_t
thread_create_c (char *name, void *(*start_routine) (void *), void *arg,
                 int line, char *file)
{
#ifndef _WIN32
  icethread_t thread;
...
#ifdef _WIN32
  ret = CreateThread (NULL, STACKSIZE, (LPTHREAD_START_ROUTINE)start_routine,
                      arg, 0, &mt->thread);
#else
  ...
  for (i = 0; i < 10; i++)
    {
      ...
      if (pthread_create (&thread, &attr, start_routine, arg) == 0)
        ...
        write_log (LOG_DEFAULT,
                   "ERROR: Could not create new thread, retrying");
    }
...
#endif
      write_log (LOG_DEFAULT,
                 "System won't let me create more threads, giving up");
      clean_shutdown (&info);
    }
  ...
  if (avl_insert (info.threads, mt))
    {
      write_log (LOG_DEFAULT,
                 "WARNING: Inserting thread resulted in duplicate.. sheit!");
    }
  internal_unlock_mutex (&info.thread_mutex);

  xa_debug (3, "DEBUG: Adding thread %d started at [%s:%d]", mt->id, file,
            line);
...
thread_create_mutex_nl (mutex_t *mutex)
{
  if (!mutex)
    {
      fprintf (stderr,
               "WARNING: thread_create_mutex_nl() called with NULL mutex\n");
      return;
    }
...
thread_create_mutex_c (mutex_t *mutex, int line, char *file)
{
  if (!mutex || !file)
    {
      fprintf (stderr,
               "WARNING: thread_create_mutex_c() called with NULL mutex\n");
      return;
    }
...
thread_mutex_lock_c (mutex_t *mutex, int line, char *file)
{
  ...
  if (!mt)
    write_log (LOG_DEFAULT, "WARNING: No mt record for %u in lock [%s:%d]",
               thread_self (), file, line);
  xa_debug (5, "Locking %p (%s) on line %d in file %s by thread %d", mutex,
            mutex_to_string (mutex, name), line, file, mt ? mt->id : -1);
  ...
  if (mt)
    {
      ...
      while ((tmutex = avl_traverse (info.mutexes, &trav)))
        {
          if (tmutex->mutexid == mutex->mutexid) /* This mutex */
            {
              if (tmutex->thread_id == mt->id) /* Deadlock, same thread can't
                                                  lock the same mutex twice */
                {
                  write_log (LOG_DEFAULT,
                             "DEADLOCK AVOIDED (%d == %d) on mutex [%s] in "
                             "file %s line %d by thread %d [%s]",
                             tmutex->thread_id, mt->id,
                             mutex_to_string (mutex, name), file, line, mt->id,
                             mt->name);
                  internal_unlock_mutex (&info.mutex_mutex);
                  return;
                }
            }
          else if (tmutex->thread_id == mt->id) /* Mutex locked by this thread
                                                   (not this mutex) */
            locks++;
        }
      ...
      if (locks > 0) /* Has already got a mutex locked */
        {
          if (info.double_mutex.thread_id
              != mt->id) /* Tries to lock two mutexes, but has not got the
                            double mutex */
            {
              write_log (LOG_DEFAULT,
                         "WARNING: (%d != %d) Thread %d [%s] tries to lock a "
                         "second mutex [%s] in file %s line %d, without "
                         "locking double mutex!",
                         info.double_mutex.thread_id, mt->id, mt->id, mt->name,
                         mutex_to_string (mutex, name), file, line);
            }
        }
      internal_unlock_mutex (&info.mutex_mutex);
    }
...
thread_mutex_unlock_c (mutex_t *mutex, int line, char *file)
{
  ...
  char name[40];
  if (!mt)
    write_log (LOG_DEFAULT, "WARNING: No mt record for %u in unlock [%s:%d]",
               thread_self (), file, line);
  xa_debug (5, "Unlocking %p (%s) on line %d in file %s by thread %d", mutex,
            mutex_to_string (mutex, name), line, file, mt ? mt->id : -1);
  ...
  if (mt)
    {
      ...
      while ((tmutex = avl_traverse (info.mutexes, &trav)))
        {
          if (tmutex->mutexid == mutex->mutexid) /* This mutex */
            {
              if (tmutex->thread_id
                  != mt->id) /* Unlocking when it's not ours */
                {
                  write_log (LOG_DEFAULT,
                             "ILLEGAL UNLOCK (%d != %d) on mutex [%s] in file "
                             "%s line %d by thread %d [%s]",
                             tmutex->thread_id, mt->id,
                             mutex_to_string (mutex, name), file, line, mt->id,
                             mt->name);
                  internal_unlock_mutex (&info.mutex_mutex);
      ...
      if ((locks > 0)
          && (info.double_mutex.thread_id
              != mt->id)) /* Don't have double mutex, has more than this mutex
                             left */
        {
          write_log (
              LOG_DEFAULT,
              "WARNING: (%d != %d) Thread %d [%s] tries to unlock a mutex "
              "[%s] in file %s line %d, without owning double mutex!",
              info.double_mutex.thread_id, mt->id, mt->id, mt->name,
              mutex_to_string (mutex, name), file, line);
        }
      internal_unlock_mutex (&info.mutex_mutex);
...
thread_exit_c (int val, int line, char *file)
{
  ...
  if (mt)
    {
      ...
      while ((tmutex = avl_traverse (info.mutexes, &trav)))
        if (tmutex->thread_id == mt->id)
          write_log (LOG_DEFAULT,
                     "WARNING: Thread %d [%s] exiting in file %s line %d, "
                     "without unlocking mutex [%s]",
                     mt->id, mt->name, file, line,
                     mutex_to_string (tmutex, name));
      internal_unlock_mutex (&info.mutex_mutex);
    }
#endif
  ...
  if (mt)
    {
      xa_debug (2,
                "DEBUG: Removing thread %d started at [%s:%d], reason: "
                "'Thread Exited'",
...
thread_catch_signals ()
{
  ...
  /* These ones should only be accepted by the signal handling thread (main
   * thread) */
  sigaddset (&ss, SIGHUP);
...
void
internal_lock_mutex (mutex_t *mutex)
{
  if (!mutex)
    {
      fprintf (stderr,
               "ERROR: internal_lock_mutex() called with NULL pointer!");
    }
  ...
  switch (pthread_mutex_lock (&mutex->mutex))
    ...
    case EDEADLK:
      fprintf (
          stderr,
          "WARNING: Locking mutex failed because a DEADLOCK would occur\n");
      break;
#endif
    }
#endif
...
void
internal_unlock_mutex (mutex_t *mutex)
{
#ifdef _WIN32
  ...
  switch (pthread_mutex_unlock (&mutex->mutex))
    ...
    case EPERM:
      fprintf (
          stderr,
          "WARNING: Unlocking mutex failed because thread was not owner\n");
      break;
#endif
    }
#endif
...
thread_setup_default_attributes ()
{
  ...
  if (pthread_attr_setdetachstate (&info.defaultattr, PTHREAD_CREATE_DETACHED)
      != 0)
...
thread_wait_for_solitude ()
{
  ...
  write_log (LOG_DEFAULT,
             "Waiting a wee while to let the other threads die..");
  ...
  write_log (LOG_DEFAULT, "Ok, that's enough, let's kill the remaining %d %s",
             avl_count (info.threads) - 1,
             avl_count (info.threads) > 2 ? "buggers" : "bugger");
}
...
n_malloc (const unsigned int size, const int lineno, const char *file)
{
  ...
  if (size <= 0)
    {
      fprintf (stderr,
               "WARNING: n_malloc called with negative or zero size\n");
      return NULL;
    }
