...
int
password_match (const char *crypted, const char *uncrypted)
{
#ifndef USE_CRYPT
  if (!crypted || !uncrypted)
    {
      write_log (LOG_DEFAULT,
                 "ERROR: password_match called with NULL arguments");
      return 0;
    }
...
int
find_frame_ofs (source_t *source)
{
  char *buff;
  int pos = 0, cid;
  ...
  if (!source)
    {
      write_log (LOG_DEFAULT,
                 "ERROR: find_frame_ofs() called with NULL argument");
      return 0;
    }
...
kick_connection (void *conarg, void *reasonarg)
{
  ...
  if (!conarg || !reasonarg)
    {
      write_log (LOG_DEFAULT,
                 "WARNING: kick_connection called with NULL pointers");
      return;
    }
  ...
  switch (con->type)
    ...
    case client_e:
      write_log (LOG_DEFAULT,
                 "Kicking client %d [%s] [%s] [%s] [%s], connected for %s, "
                 "%lu bytes transfered. %lu clients connected",
                 con->id, nullcheck_string (con->user), con_host (con), reason,
                 con->food.client->type == listener_e ? "listener" : "relay",
    ...
    case source_e:
      write_log (LOG_DEFAULT,
                 "Kicking source %d [%s] [%s] [%s], connected for %s, %lu "
                 "bytes transfered. %d sources connected",
                 con->id, con_host (con), reason,
                 con->food.source->type == encoder_e ? "encoder" : "relay",
                 nice_time (get_time () - con->connect_time, timebuf),
                 con->food.source->stats.read_bytes, info.num_sources - 1);
      ...
      break;
    default:
      write_log (LOG_DEFAULT, "Kicking unknown %d [%s] [%s], connected for %s",
                 con->id, con_host (con), reason,
                 nice_time (get_time () - con->connect_time, timebuf));
      break;
    }
...
close_connection (void *data, void *param)
{
  ...
  if (!con)
    {
      write_log (LOG_DEFAULT,
                 "ERROR: close_connection called with null pointer!");
      return;
    }
  ...
  if (con->type == client_e)
    {
      ...
      if (con2)
        {
          con2->stats.client_connect_time
              += (unsigned long)((get_time () - con->connect_time) / 60.0);
          info.hourly_stats.client_connect_time
              += (unsigned long)((get_time () - con->connect_time) / 60.0);

          xa_debug (2,
                    "DEBUG: Removing client %d (%p) from sourcetree of (%p)",
                    con->id, con, con2);
  ...
  else if (con->type == source_e)
    {
      ...
      if (!source)
        {
          write_log (LOG_DEFAULT,
                     "WARNING!!! - Erroneous source without food");
          return;
        }

      xa_debug (2, "Removing source %d (%p) from sourcetree of (%p)", con->id,
                con, info.sources);
      ...
      info.hourly_stats.source_connect_time
          += ((get_time () - con->connect_time) / 60);
      ...
      if (source->source_agent != NULL)
        nfree (source->source_agent);
...
kick_not_connected (connection_t *con, char *reason)
{
  ...
  if (reason)
    write_log (LOG_DEFAULT, "Kicking %s %d [%s] [%s], connected for %s",
               type_of_str (con->type, typebuf), con->id, con_host (con),
               reason, nice_time (get_time () - con->connect_time, timebuf));
...
kill_threads ()
{
  ...
  write_log (LOG_DEFAULT,
             "Closing sockets for sources that keep hanging around...");
...
write_icecast_header ()
{
  ...
  printf ("NtripCaster comes with NO WARRANTY, to the extent permitted by "
          "law.\nYou may redistribute copies of NtripCaster under the terms "
...
print_startup_server_info ()
{
  ...
  write_log (LOG_DEFAULT,
             "Server limits: %d clients, %d clients per source, %d sources",
             info.max_clients, info.max_clients_per_source, info.max_sources);
}

char *
connect_average (unsigned long int minutes, unsigned long int connections,
                 char *buf)
{
  if (!connections)
    return nice_time_minutes (minutes, buf);
  return nice_time_minutes (
      (unsigned long int)((double)minutes / (double)connections), buf);
}
...
hostname_local (char *name)
{
  ...
  if (ice_strcasecmp (name, "localhost") == 0
      || ice_strcasecmp (name, "127.0.0.1") == 0)
    return 1;
  ...
  /* Not in the tree, try to reverse it */
  {
    ...
    if (out
        || (info.myhostname && (ice_strcasecmp (res, info.myhostname) == 0))
        || (ice_strcmp (res, "127.0.0.1") == 0))
      {
        thread_mutex_lock (&info.hostname_mutex);
...
build_request (char *line, request_t *req)
{
  ...
  else if ((ice_strncmp (line, "HOST:", 5) == 0)
...
generate_http_request (char *line, request_t *req)
{
  ...
  if (!line || !req)
    {
      write_log (LOG_DEFAULT,
                 "ERROR: generate_request called with NULL pointer");
      return;
    }
  ...
  xa_debug (2, "DEBUG: Generated http request [%s:%d%s]", req->host, req->port,
            req->path);
}
...
void
init_thread_tree (int line, char *file)
{
  mythread_t *mt;
  ...
  if (!file)
    {
      fprintf (stderr,
               "WARNING: init_thread_tree() called with file == NULL\n");
      exit (1);
    }
  ...
  if (avl_insert (info.threads, mt))
    {
      fprintf (stderr, "WARNING: Could not insert main thread into the thread "
                       "tree, DAMN!\n");
      exit (1);
    }
...
pending_connection (connection_t *con)
{
  write_log (
      LOG_DEFAULT,
      "Lost connection to source on mount %s, waiting %d seconds for timeout",
...
pending_source_signoff (connection_t *con)
{
  ...
  while ((running == SERVER_RUNNING)
         && con->food.source->connected == SOURCE_PENDING
         && ((get_time () - start) < info.client_timeout))
    my_sleep (90000);
...
get_icecast_file (const char *filename, filetype_t type, int flags)
{
  ...
  switch (type)
    {
    case conf_file_e:
      snprintf (path_and_file, BUFSIZE, "%s%c%s", info.etcdir, DIR_DELIMITER,
                filename);
      break;
    case log_file_e:
      snprintf (path_and_file, BUFSIZE, "%s%c%s", info.logdir, DIR_DELIMITER,
                filename);
      break;
    default:
  ...
  switch (type)
    {
    case conf_file_e:
      snprintf (path_and_file, BUFSIZE, ".%c%s%c%s", DIR_DELIMITER, "conf",
                DIR_DELIMITER, filename);
      break;
    case log_file_e:
      snprintf (path_and_file, BUFSIZE, ".%c%s%c%s", DIR_DELIMITER, "logs",
                DIR_DELIMITER, filename);
      break;
    default:
      snprintf (path_and_file, BUFSIZE, "%s", filename);
...
my_sleep (int microseconds)
{
  ...
  switch (nanosleep (&req, &rem))
    ...
    case EINVAL:
      xa_debug (1,
                "WARNING: nanosleep() was passed invalid or negative sleep "
                "value %ld+%ld",
...
set_element configfile_settings[] = {
  ...
  { "max_clients_per_source", integer_e,
    "Max number of clients listening on one source", NULL },
  ...
  while (fd_read_line (cf, line, BUFSIZE) > 0)
    {
      ...
      if (splitc (word, line, ' ') == NULL)
        {
          write_log (LOG_DEFAULT,
                     "ERROR: No argument given to setting %s on line %d", line,
                     lineno);
          continue;
        }
      ...
      if (!se)
        {
          write_log (LOG_DEFAULT, "Unknown setting %s on line %d", word,
                     lineno);
          continue;
        }
...
// compute distance
double
compute_distance_xyz (double pos1[3], double pos2[3])
{
...
void
llh2xyz (const double latitude, const double longitude, const double height,
         double (*r_xyz)[3])
{
  double a = 6378137;
...
void
get_mount_location_from_file (char *file, char *mount, pos_t *pos)
{
  int mf;
  char line[BUFSIZE];
  int lineno = 0;
  xa_debug (1, "DEBUG: Parsing mount location file %s",
            file ? file : "(null)");
  if (!file)
    {
      return;
...
void
get_mount_location (char *line, pos_t *mp)
{
  char item[BUFSIZE];
...
write_http_header (sock_t sockfd, int error, const char *msg)
{
  ...
  sock_write_line (sockfd, "Server: NTRIP NtripCaster %s/%s", info.version,
                   info.ntrip_version);
}
