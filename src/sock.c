...
int
sock_valid (const SOCKET sockfd)
{
  return ((int)sockfd >= 0);
}
 ...
 */
int
sock_set_keepalive (SOCKET sockfd, const int keepalive)
{
  ...
  res = setsockopt (sockfd, SOL_SOCKET, SO_KEEPALIVE, (void *)&optval,
                    sizeof (int));
    ...
    if (!is)
      {
        write_log (LOG_DEFAULT,
                   "WARNING: sock_set_keepalive() setting unknown socket");
      }
    else
      {
...
#ifdef SO_LINGER
int
sock_set_no_linger (SOCKET sockfd)
{
  ...
  res = setsockopt (sockfd, SOL_SOCKET, SO_LINGER, (void *)&lin,
...
#ifdef DEBUG_SOCKETS
  {
    ...
    if (!is)
      {
        write_log (LOG_DEFAULT,
                   "WARNING: sock_set_no_linger() setting unknown socket");
      }
    else
      {
        is->linger = 0;
 ...
 */
int
sock_set_blocking (SOCKET sockfd, const int block)
{
#ifdef _WIN32
  int varblock = block;
#else
  ...
  if (!sock_valid (sockfd))
    {
      xa_debug (1, "ERROR: sock_set_blocking() called with invalid socket");
      return SOCKET_ERROR;
    }
...
#ifdef DEBUG_SOCKETS
  {
    ...
    if (!is)
      {
        write_log (LOG_DEFAULT,
                   "WARNING: sock_set_blocking(): Unknown socket %d", sockfd);
        return -1;
      }
    else
      {
...
sock_insert (ice_socket_t *is)
{
  ...
  if (avl_insert (sock_sockets, is))
    {
      write_log (LOG_DEFAULT,
                 "WARNING: Tried to insert duplicate socket. Weird!");
      thread_mutex_unlock (&sock_mutex);
      return 0;
    }
...
SOCKET
sock_socket (int domain, int type, int protocol)
{
...
SOCKET
sock_accept (SOCKET s, struct sockaddr *addr, mysocklen_t *addrlen)
{
                          ...
                          const char *buff, const int len)
{
  ...
  if (!sock_valid (sockfd))
    {
      xa_debug (
          1, "ERROR: sock_write_bytes_or_kick() called with invalid socket");
      return -1;
    }
  else if (!clicon)
    {
    ...
    {
      xa_debug (1, "ERROR: sock_write_bytes_or_kick() called with NULL data");
      return -1;
    }
  else if (len <= 0)
    {
      xa_debug (
          1, "ERROR: sock_write_bytes_or_kick() called with invalid length");
      return -1;
    }
  ...
  if (res < 0)
    {
      xa_debug (4, "DEBUG: sock_write_bytes_or_kick: %d err [%d]", res, err);
 ...
 */
int
sock_write_bytes (SOCKET sockfd, const char *buff, int len)
{
  int t;

  if (!buff)
    {
      xa_debug (1, "ERROR: sock_write_bytes() called with NULL data");
      return -1;
    }
  else if (len <= 0)
    {
    ...
    {
      xa_debug (1, "ERROR: sock_write_bytes() called with invalid socket");
      return -1;
    }
 ...
 */
int
sock_write_string (SOCKET sockfd, const char *buff)
{
  ...
  if (!sock_valid (sockfd))
    ...
    {
      fprintf (stderr, "ERROR: sock_write_string() called with NULL format\n");
      return -1;
    }
  ...
  if (sockfd == 1 || sockfd == 0)
  ...
  else
    {
      while (write_bytes < len)
        {
          res = send (sockfd, &buff[write_bytes], len - write_bytes, 0);
 ...
 */
int
sock_write (SOCKET sockfd, const char *fmt, ...)
{
  char buff[BUFSIZE];
  va_list ap;
 ...
 */
int
sock_write_line (SOCKET sockfd, const char *fmt, ...)
{
  char buff[BUFSIZE];
  va_list ap;
 ...
 */
int
sock_read_lines_np (SOCKET sockfd, char *buff, const int len)
{
  char c = '\0';
  ...
  if (!sock_valid (sockfd))
    {
      xa_debug (1, "ERROR: sock_read_lines_np() called with invalid socket");
      return 0;
    }
  else if (!buff)
    {
      xa_debug (
          1, "ERROR: sock_read_lines_np() called with NULL storage pointer");
      return 0;
    }
  else if (len <= 0)
    {
      xa_debug (1, "ERROR: sock_read_lines_np() called with invalid length");
      return 0;
    }
#ifdef _WIN32
  WSASetLastError (0);
...
int
sock_read_lines (SOCKET sockfd, char *buff, const int len)
{
  char c[2];
  int read_bytes, pos = 0;
  ...
  if (!sock_valid (sockfd))
    {
      xa_debug (1, "ERROR: sock_read_lines() called with invalid socket");
      return 0;
    }
  else if (!buff)
    {
    ...
    {
      xa_debug (1, "ERROR: sock_read_lines() called with invalid length");
      return 0;
    }
 ...
 * If info.myhostname is NULL, bind it to INADDR_ANY (all available
 * interfaces). If info.myhostname is not NULL, resolv it (if needed), and bind
 * to that. Return the socket for bound socket, or INVALID_SOCKET if failed.
 * Assert Class: 3
 */
SOCKET
sock_get_server_socket (const int port)
{
  ...
  if (port < 0)
    {
      write_log (LOG_DEFAULT,
                 "ERROR: Invalid port number %d. Cannot listen for requests, "
                 "this is bad!",
                 port);
...
#ifdef HAVE_SETSOCKOPT
  {
    ...
    if (setsockopt (sockfd, SOL_SOCKET, SO_REUSEADDR, (const void *)&tmp,
                    sizeof (tmp))
        != 0)
      write_log (LOG_DEFAULT, "ERROR: setsockopt() failed to set SO_REUSEADDR "
                              "flag. (mostly harmless)");
  }
#endif
  /*
  ...
  if (info.myhostname != NULL && info.myhostname[0])
    {
      if (isdigit ((int)info.myhostname[0])
          && isdigit ((int)info.myhostname[ice_strlen (info.myhostname) - 1]))
        {
          if (inet_aton (info.myhostname, (struct in_addr *)&localaddr) == 0)
            {
              write_log (LOG_DEFAULT,
      ...
      else
        {
          ...
          hostinfoptr = ice_gethostbyname (info.myhostname, &hostinfo, buf,
                                           BUFSIZE, &error);
          if (hostinfoptr == NULL)
            {
              write_log (LOG_DEFAULT, "Unknown host %s, that's it for me!",
                         info.myhostname);
              ice_clean_hostent ();
              clean_shutdown (&info);
            }
          sin.sin_addr.s_addr = localaddr.s_addr;
          memcpy ((void *)&localaddr, hostinfoptr->h_addr, sizeof (localaddr));
          ice_clean_hostent ();
        }
    }
  else
  ...
  if (info.myhostname != NULL)
    {
      ...
      if (bind (sockfd, (struct sockaddr *)&localsin, sizeof (localsin))
          == SOCKET_ERROR)
        {
          xa_debug (2, "DEBUG: Unable to bind", info.myhostname);
          write_log (LOG_DEFAULT, "ERROR: Bind to local address %s failed",
                     info.myhostname);
          sock_close (sockfd);
          return INVALID_SOCKET;
        }
  ...
  if (isdigit ((int)hostname[0])
      && isdigit ((int)hostname[ice_strlen (hostname) - 1]))
    {
      if (inet_aton (hostname, (struct in_addr *)&sin.sin_addr) == 0)
        {
          write_log (LOG_DEFAULT, "ERROR: Invalid ip number %s", hostname);
          sock_close (sockfd);
          return INVALID_SOCKET;
        }
    ...
    {
      host = ice_gethostbyname (hostname, &hostinfo, buf, BUFSIZE, &error);
      if (host == NULL)
        {
  ...
  if (timeout > 0)
    {
      ...
      if (retval == 0)
        {
          xa_debug (3,
                    "DEBUG: sock_connect(): non blocking connect returned 0!");
...
#endif
              xa_debug (3,
                        "DEBUG: sock_connect(): connect didn't return "
                        "EINPROGRESS!, was: %d",
                        errno);
              sock_close (sockfd);
              return SOCKET_ERROR;
      ...
      if (select (sockfd + 1, NULL, &wfds, NULL, &tv))
        {
          ...
          if ((retval == 0) && (val == 0))
            ...
            {
              xa_debug (3,
                        "DEBUG: sock_connect(): getsockopt returned %i, val = "
                        "%i, valsize = %i, errno = %i!",
...
static const char libwraptype[4][20]
    = { "NtripCaster_client", "NtripCaster_source", "NtripCaster_admin",
        "NtripCaster" };

/*
 * Return the corresponding string for libwrap for the specified connection
 * type. Assert Class: 1
 */
const char *
sock_get_libwrap_type (const contype_t contype)
{
  switch (contype)
    {
    case client_e:
 ...
 */
int
sock_check_libwrap (const int sock, const contype_t contype)
{
  struct request_info req;

  if (!sock_valid (sock))
    {
      xa_debug (1, "ERROR: sock_check_libwrap() called with invalid socket");
      return 0;
    }
