...
int sock_insert(ice_socket_t *is)
{
...
int sock_add(SOCKET s, int domain, int type, int protocol)
{
	ice_socket_t *is;
...
SOCKET sock_get_server_socket(const int port)
{
	...
	{
		if (isdigit((int)info.myhostname[0]) && isdigit((int)info.myhostname[ice_strlen(info.myhostname) - 1]))
		{
			if (inet_aton(info.myhostname,
						...
						const int timeout)
{
	...
	if (info.myhostname != NULL)
	{
		...
		if (bind(sockfd, (struct sockaddr *)&localsin,
	...
	if (isdigit((int)hostname[0]) && isdigit((int)hostname[ice_strlen(hostname) - 1]))
	{
