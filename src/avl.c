...
int avl_count(const avl_tree *tree)
{
	assert(tree != NULL);
	return tree->count;
}
...
void print_structure(avl_node *node, int level)
{
	char lc[] = "([{`/";
	char rc[] = ")]}'\\";
...
int compare_ints(const void *a, const void *b, void *param unused)
{
	return ((int)a) - ((int)b);
}

void print_int(void *a, void *param unused)
{
	printf(" %d", (int)a);
}

int recurse_tree(avl_node *node, int *count, int parent, int dir)
{
	if (node)
	{
		int d = (int)node->data;
...
void verify_tree(avl_tree *tree)
{
	int count = 0;
...
void compare_trees(avl_node *a, avl_node *b)
{
	if (a == NULL || b == NULL)
	{
	...
	if (a->data != b->data || a->bal != b->bal || ((a->link[0] != NULL) ^ (b->link[0] != NULL)) || ((a->link[1] != NULL) ^ (b->link[1] != NULL)))
	{
...
#ifdef DEBUG_MEMORY
int compare_mem(const void *firstp, const void *secondp, void *param)
{
