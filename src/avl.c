...
avl_tree *
avl_create_nl (MAYBE_ARENA avl_comparison_func cmp, void *param)
{
  avl_tree *tree;
...
#define new_node(owner) new_node ()
#endif
  ...
  if (!first || !second)
    {
      write_log (LOG_DEFAULT,
                 "WARNING: compare_strings called with null pointers");
      return 0;
    }
...
compare_threads (const void *first, const void *second, void *param)
{
  ...
  if (!first || !second)
    {
      write_log (LOG_DEFAULT,
                 "WARNING: compare_threads called with NULL pointers");
      return 0;
    }
...
compare_mutexes (const void *first, const void *second, void *param)
{
  ...
  if (!first || !second)
    {
      write_log (LOG_DEFAULT,
                 "WARNING: compare_mutex called with NULL pointers");
      return 0;
    }
...
int
compare_connection (const void *first, const void *second, void *param)
{
  connection_t *a1, *a2;
  ...
  if (a1->type != a2->type)
    {
      write_log (LOG_DEFAULT,
                 "WARNING!!!! - Comparing different type connections");
      return -1;
    }
...
compare_sockets (const void *first, const void *second, void *param)
{
  ...
  if (is1 == NULL || is2 == NULL)
    ...
    {
      fprintf (stderr,
               "WARNING: compare_sockets called with negative socket number");
      return -1;
    }
...
avl_get_any_node (avl_tree *tree)
{
  ...
  if (!tree)
    {
      write_log (LOG_DEFAULT,
                 "WARNING: avl_get_any_node called with NULL tree");
      return NULL;
    }
